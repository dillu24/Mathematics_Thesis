...
It is important to note that there are different ways how the ACO framework can be applied to the TSP. The ACO framework described in the previous paragraphs is known as the Ant Colony System (ACS) in literature. The ACS was chosen because, it is one of the most empirically successful frameworks in terms of accuracy of approximations \cite{dorigo_stutzle_thomas_2004}. In addition to this, as it will be seen later in this section, the main theorem of this project also applies to the ACS. It is important to note that later on in this section, other variants of the ACO frameworks which can be applied to the TSP will be briefly described by discussing how they vary from the ACS. Note that in this project, the terms ACO framework, ACO algorithm, and ACO metaheuristic variant will be used interchangeably in this write up. The discussion will now proceed by presenting the pseudocode of the algorithm for the TSP which follows the ACS framework. Note that Algorithm \ref{ACO_ALG} was constructed using ideas from \cite{dorigo_gambardella_1997}. In Algorithm \ref{ACO_ALG} below, $G(V,E,f)$ is an instance of the TSP, and $g$ is a probabilistic function with the properties described in the previous paragraph. Also, $a.current\_vertex$ is the variable containing the vertex the ant $a$ is currently on.
\begin{algorithm}[H]
\begin{algorithmic}[1]
...
\end{theorem}
\begin{proof}
Let $p_{min}$ be the probability that an ant $k$ on component $c_i$ chooses component $c_j$ such that the partial solution $x_h$= $(v_1, v_2, ..., v_i = c_i, ..., v_j = c_j)$ is constructed. Since by Lemma \ref{lemma1_aco}, and by Lemma \ref{lemma2_aco} $\tau_{min}$ and $\tau_{max}$ exist, then from Formula \ref{eq:gen1} it can be deduced that $p_{min}$ $>$ 0. The smallest value that $p_{min}$ can be is represented by the situation when the pheromone on the edge chosen is $\tau_{min}$,  and all other edges have $\tau_{max}$ pheromone on them. In other words, the smallest value of $p_{min}$ can be represented by $\widetilde{p}_{min}$ = $\displaystyle{\frac{\tau_{min}^\alpha}{(N-1)\tau_{max}^\alpha + \tau_{min}^\alpha}}$, where $N$ is the number of components. Now, by definition of $\widetilde{p}_{min}$, $\widetilde{p}_{min}$ $\leq$ $p_{min}$. Let $\widetilde{p}$ be the probability of any solution $s^\prime$ to be generated by the $ACO_{bs, t_{min}}$ algorithm. Then, $\widetilde{p}$ $\geq$ $\widetilde{p}_{min}^n$ where $n$ is the maximum length of $s$. Where this happens when all components in $s^\prime$ are chosen with probability $\widetilde{p}_{min}$. Since this holds for all $s^\prime$ $\in$ $S$, it holds for $s^\star$ (therefore $\widetilde{p}$ is the probability of $s^\star$ being generated). Therefore, the probability of finding $s^\star$ exactly once in the first $\theta$ iterations is equal to not finding it $\theta$ -1 times and finding it only one time. Therefore, denoting bla bla continue..  say it is a lower bound for P*(theta).. note billi p frac tigi up ta theta minajr -1 .. the probability canbe made larger if we make theta arbitrarily larg.. epsilon equals dak li jifdal.. allura l limit imur tajjeb  
\end{proof}

%configuration do it after theory
...
%From this statement, one can deduce that the time the ACS is allowed to execute is critical for the ACS algorithm. In addition to this, there are no theoretical results about the speed of convergence of any ACO algorithm. Therefore, the algorithmic performance in terms of speed of convergence of any ACO algorithm can only be measured empirically, as it will be done in Chapter 3. \cite{dorigo_stutzle_thomas_2004}% itfaha lahahr wahda wara t theorem din
