...
A problem can be verified in polynomial time if given an arbitrary solution to the problem, the solution can be checked by an algorithm if it is correct or not in polynomial time. {\normalfont{\cite{cormen_leiserson_rivest_stein}}}
\end{definition}
...
As discussed in previous paragraphs, Lemma \ref{distinct_hamiltonian_cycles} confirms that a brute-force algorithm will not be efficient enough to solve the TSP for large instance sizes. This means that, we can either try and design a new algorithm that solves the TSP in polynomial time, or, try and prove that TSP is an NP-Complete problem. The latter would confirm that no polynomial time algorithm is known for the TSP. However, at the same time this does not exclude that a polynomial time algorithm that solves TSP does not exist.\\\\ In what follows, it will be proven that the TSP is an NP-Complete problem. Therefore, first it will be shown that TSP is in NP, and secondly, that TSP is as hard as any problem in NP (as discussed in section 1.2). It is important to note that in order to prove that TSP is in NP, TSP must be converted into a decision problem. This is needed because in Definition \ref{TSP}, TSP was presented as an optimisation problem. The TSP's related decision problem is the following :\\\\
Given a complete weighted graph $G$ and an integer $k$, does $G$ contain a Hamiltonian cycle of weight at most $k$ \cite{cormen_leiserson_rivest_stein}?\\\\
Having converted the TSP from an optimisation problem into a decision problem, it is now time to show that TSP is in NP. This is shown in lemma \ref{TSP_in_NP} below.
\begin{theorem}
\label{TSP_in_NP}
TSP is in NP {\normalfont{\cite{cormen_leiserson_rivest_stein}}}.
\end{theorem}
\begin{proof}
A solution to the TSP given a weighted graph $G(V,E)$ and an integer $k$ as instances, is a sequence of $n+1$ vertices representing a Hamiltonian cycle of weight at most $k$. Suppose that $v_1$, $v_2$, ..., $v_n$, $v_1$ is an arbitrary solution to the TSP instance. The solution can be verified by checking that no vertex is repeated excluding $v_1$, and that the total weight of the Hamiltonian cycle is at most $k$ (note by checking that no vertex is repeated, the solution sequence is confirmed to be a Hamiltonian cycle). Therefore, the verification algorithm is made up of the following 2 parts:
\begin{itemize}
   \item $\forall$ $i$ $\in$ [$n$], the verificaiton algorithm checks that $v_i$ appears only once in the sequence $v_1$, ..., $v_n$. This part of the algorithm can be implemented by, keeping an array $A$ of boolean values having length $n$ such that, for each $v_i$, the algorithm checks that $A[i]$ is $0$. If $A[i]$ is set to $0$, the algorithm sets $A[i]$ to $1$ and moves to the next vertex in the solution sequence. If while traversing the solution sequence, the verification algorithm notices that for some $i$ $\in$ [$n$], $A[i]$=1, this means that $v_i$ is repeated and hence, the verification algorithm stops since the solution is not correct. Otherwise, the algorithm starts executing the next part (next bullet point). Clearly, this part of the algorithm has a worst-case $O(n)$ time complexity were $n$ is the length of the solution sequence. The worst-case occurs when the verification algorithm needs to traverse all the vertices in the solution sequence. As a result, this part of the verification algorithm can be executed in polynomial time.
   \item The verification algorithm must now check that, the total weight of the Hamiltonian cycle represented by the sequence of vertices is at most $k$ . This part of the verification algorithm can be implemented by keeping a floating point variable named `total\_weight', and summing the weight of each edge used in the Hamiltonian cycle to the variable `total\_weight'. Since a Hamiltonian cycle contains $n$ edges, this part of the verification algorithm has a worst-case $O(n)$ time complexity. After the total weight of the solution sequence is calculated, it is checked in $O(1)$ time whether `total\_weight' $\leq k$. If this inequality is satisfied, the verification algorithm declares the solution as correct, otherwise declares the solution as incorrect. Therefore, it can be concluded that, this part of the verification algorithm has an $O(n)$ worst-case time complexity. Hence, this part of the verification algorithm can be executed in polynomial time.
\end{itemize} 
Since the verification algorithm is made up of two parts that execute in polynomial time, then, the whole algorithm runs in polynomial time. As a result, since the solution sequence is arbitrary, the TSP can be verified in polynomial time. Therefore, TSP is in NP. 
\end{proof}
