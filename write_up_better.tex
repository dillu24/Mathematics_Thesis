...
\begin{definition}
\label{NPC}
A decision problem P is in the class NP-Complete (NPC) if P is in NP and P is as hard as any problem in NP. {\normalfont{\cite{cormen_leiserson_rivest_stein}} }
\end{definition}
In other words, definition \ref{NPC} states that for a problem $\mathit{P}$ to be in NPC, $\mathit{P}$ must be in NP and that any problem in NP can be transformed using a reduction algorithm into $\mathit{P}$. Therefore, to show that a problem $\mathit{P}$ is in NPC, one must show that it is in NP, and that every problem in NP can be reduced to $\mathit{P}$. Showing that $\mathit{P}$ is in NP is not difficult because, it only requires to show that an arbitrary solution of $\mathit{P}$ can be verified in polynomial time. On the other hand, showing that every problem in NP is reducible to $\mathit{P}$ may seem diffcult because, this requires checking that every problem in NP can be reduced to $\mathit{P}$ \cite{geeksforgeeks_2018_2}. However, this is not the case because reduction algorithms are transitive \cite{geeksforgeeks_2018_2}. Hence, considering a known NP-Complete problem $\mathit{H}$, since $\mathit{H}$ is NP-Complete, every problem in NP can be reduced to $\mathit{H}$. Therefore, if one can find a reduction algorithm from $\mathit{H}$ to $\mathit{P}$, then by transitivity of reduction algorithms, every problem in NP can be reduced to $\mathit{P}$ using a reduciton algorithm \cite{geeksforgeeks_2018_2}.\\\\
The classes P, NP and NPC have very important implications in computational theory. One of the most important problems in computational theory is whether P = NP. It is known that P $\subseteq$ NP. This is true because, since every problem in P can be solved in polynomial time, then implicitly the polynomial time verification has been done by the algorithm solving the problem. Therefore, every problem in P is in NP and hence, P $\subseteq$ NP. Showing that NP $\subseteq$ P may seem to be easy aswell. In fact, to show that NP $\subseteq$ P, one only needs to show that $\exists$ an NP-Complete problem $\mathit{C}$ that can be solved in polynomial time. This is true because, by the NP-Completness of $\mathit{C}$, every problem in NP can be reduced using a reduction algorithm to $\mathit{C}$ and hence, every problem in NP could be solved in polynomial time since the reduction algorithm and the algorithm solving $\mathit{C}$, would take a total time that is still polynomial. However, for problems in NPC, no polynomial time algorithm has yet been discovered. In addition to this, no one has yet been able to prove that such polynomial time algorithms do not exist for NP-Complete problems. This essentially means that, the NP $\subseteq$ P problem is still an open question. \cite{cormen_leiserson_rivest_stein}\\\\
...
\end{example}
Example \ref{example_2.1} may show that, to solve the TSP on a graph instance, one can do it by a brute-force algorithm. A brute-force algorithm for TSP is one that computes all the Hamiltonian cycles in a graph and chooses the one of least weight as a soltution. However, a brute-force algorithm is inefficient because a complete graph on at least three vertices contains $\mathit{\frac{(n-1)!}{2}}$ distinct Hamiltonian cycles and hence, since, the brute-force algorithm must generate all the distinct Hamiltonian cycles in the graph, the brute-force algorithm must take factorial time to generate all Hamiltonian cycles. This means that the brute-force algorithm is a factorial time algorithm. \cite{Sanchit}, \cite{mathematics_stack_exchange_2012}
\\\\The following lemma confirms that the complete graph $\mathit{K_n, n\geq 3}$, contains $\mathit{\frac{(n-1)!}{2}}$ distinct Hamiltonian cycles. Therefore, confirming that a brute-force algorithm cannot efficiently solve the TSP.
we can either try and find an algorthm or show that tsp is np complete after theorem
