...
Let G(V,E,f) be a complete weighted graph on $n$ vertices, and $W^\star$ be the optimal value of the Metric-TSP defined on G. Suppose that there exists a mapping g : V $\mapsto$ $\real$, g(v) = $l_v$, such that the following  two conditions hold:
\begin{itemize}
\item $f(\{u, v\})$ $\geq$ min( $l_u$, $l_v$) $\forall$ $u$,$v$ $\in$ $V$
\item $l_u$ $\leq$ $\frac{1}{2}W^\star$ $\forall$u $\in$ V
\end{itemize}
...
\begin{claim}
$W^\star$ $\geq$ 2 $\sum_{i = k+1}^{min(2k, n)} l_i$, $\forall$ $k$ $\in$ $\mathbb{N}$ satisfying 1 $\leq$ $k$ $\leq$ $n$.
\end{claim}
Let $H(V^\prime,E^\prime,f)$ be a complete weighted subgraph of $G$ such that, $V^\prime$ = $\{i | 1 \leq i \leq min(2k, n)\}$. Let $T$ = $v_1$, $v_2$, ..., $v_{min(2k, n)}$, $v_1$ be a Hamiltonian cycle in $H$ whose sequence of vertices are in the same order as they would appear in a minimum weight Hamiltonian cycle of $G$. Let $T^\star$ be the weight of $T$ when constructed as a cycle subgraph $C(V^\prime, E^{\prime\prime}, f)$ of $H$. By the triangle inequality, for all $\{v_i, v_j\}$ in $E^{\prime\prime}$, $f(\{v_i, v_j\})$ is less than or equal to the weight of the path from $u$ to $v$ in a minimum weight Hamiltonian cycle of $G$. Therefore, $T^\star$ $\leq$ $W^\star$. Now, $T^\star$ =  $\sum_{\{i, j\} \in E^{\prime\prime}} f(\{i, j\})$. Therefore, by the first bullet point in Lemma \ref{to_proove_bound},  $T^\star$ = $\sum_{\{i, j\} \in E^{\prime\prime}} f(\{i, j\})$ $\geq$  $\sum_{\{i, j\} \in E^{\prime\prime}} min(l_i, l_j)$. Let $\alpha_i$ be the number of edges in $E^{\prime\prime}$ for which $i > j$ ( i.e $l_i$ = $min(l_i, l_j)$). Then $\sum_{\{i, j\} \in E^{\prime\prime}} min(l_i, l_j)$ = $\sum_{i \in E^\prime} \alpha_il_i$\\
$\implies$ $T^\star$ $\geq$ $\sum_{i \in E^\prime} \alpha_i l_i$ = $\sum_{i =1}^{min(2k, n)} \alpha_i l_i$ $\geq$ $\sum_{i = k+1}^{min(2k, n)} \alpha_i l_i$. Now, for all $i$ $\in$ [$min(2k, n)$], $\alpha_i$ $\leq$ 2. This is because, there are exactly two edges incident to each vertex $i$ in $C$. In addition to this, $\alpha_1$ + .. + $\alpha_{min(2k, n)}$ = $\abs{E^{\prime\prime}}$  .. now suppose min(2k, n) = k u kompli kif tigi l bound.. tigi greater or equal it tnjen suppost
\end{proof}
\begin{theorem}
