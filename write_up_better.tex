...
\end{lemma}
\begin{proof}
Suppose that an optimal solution $s^\star$ is found by the $ACO_{bs, \tau_{min}}$ algorithm. Since no other solution $s$ $\in$ $\widetilde{S}$ has $f(s)$ less than $s^\star$, then, after performing Formula \ref{eq:gen2}, the $ACO_{bs, \tau_{min}}$ algorithm will keep on performing Formula $\ref{eq:gen3}$ on $s^\star$ (*). This is true because, after performing pheromone evaporation, the $ACO_{bs, \tau_{min}}$ updates the pheromones in $\mathrm{T}$ using the global-best pheromone update procedure. In addition to this, $\tau_{ij}^\star(\theta)$ $\geq$ $\tau_{min}$ $>$ 0 (**). As a result, (*) and (**) together imply that $\tau_{ij}^\star(\theta)$ is monotonically increasing. Let $\theta ^\star$ be the iteration in which $s^\star$ is first found. Then, the proof of Lemma $\ref{lemma1_aco}$ can be used to obtain the desired result by letting $\tau_0$ be equal to $\tau_{ij}^\star(\theta^\star)$.
\end{proof}
