...
The Ant Colony Optimization Algorithm (ACO) forms part of the metaheuristic class of algorithms \cite{dorigo_stutzle_thomas_2004}. A metaheuristic is a set of algorithmic notions that can be used to define methods that solve a wide set of problems \cite{dorigo_stutzle_thomas_2004}. In other words this means that, the ACO is a framework that can be applied to a number of problems. Note that, this is different than what have been presented in Section 2.2.1 and Section 2.2.2 because, the TAMSA and the NNA are algorithms that are specifically written for the TSP. Hence, they cannot be applied to other combinatorial problems. Algorithms such as the NNA and the TAMSA, which are specifically written to approximate a problem are loosely called heuristics \cite{dorigo_stutzle_thomas_2004}. Therefore, it can be concluded that, metaheuristics are at a higher level of abstraction than heuristics because, they can be used to define heuristics.\\\\
A metaheuristic normally has a general idea which can then be extended in order to be implemented as an algorithm for a specific problem. The ACO's general idea was inspired from the behavior of ants. Ants are capable of finding the shortest path from a food source to their nest and vice-versa without using any visual indications. The ants manage to find shortest paths due to the following. Each ant deposits a chemical known as pheromone on the path from a food source to it's nest. In addition to this, when an ant has a choice in which direction it should move, it probabilistically chooses a direction which has rich pheromone levels. Obviously, ants that choose shorter paths will arrive to their nest much quicker than the ants that choose longer paths. This means that, on the way back to the food source, the shortest path has more pheromone than any other path because, the other paths would have not yet been constructed. Therefore, on their way back to the food source, the ants would probabilistically choose the shortest path. In addition to this, pheromone evaporates. Hence, the longer a path is, the more evaporation it experiences because, ants take longer to reach their destination. As a result, there is less pheromone present on longer paths, hence, the ants will eventually ignore that path. After a number of repetitions, the above procedure would eventually result in a situation were all ants choose the shortest path from their nest to a food source and vice-versa. \cite{dorigo_gambardella_1997}\\\\
To understand the ACO framework better, Example \ref{explain_ACO} will demonstrate how the ACO framework can be applied to the Shortest Path Problem, without going into specific details on how the algorithm can be implemented.
\begin{example}
\label{explain_ACO}
\normalfont{Suppose} that the shortest path of the graph $G(V,E,f)$ below from vertex $v_1$ to vertex $v_4$ must be found.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
...
\end{tikzpicture}
\\\\
Clearly, the ACO framework can be applied to the Shortest Path Problem on $G$ by letting the ant's environment be $G$, and the food source and the nest be $v_1$ and $v_2$ respectively. The ants would then find the shortest path due to the following. Since no ant has yet started to walk in it's environment, no pheromone has yet been deposited. Hence at vertex $v_1$, $v_2$ and $v_3$ have the same probability of being chosen by ants. Hence, probabilistically, half of the ants move to $v_2$ and half of the ants move to $v_3$.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
...
\end{tikzpicture}
\\\\
Ants at $v_3$ and $v_2$ have only one vertex to visit because $v_1$ has already been visited. Therefore, all ants move to vertex $v_4$.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
...
\end{tikzpicture}
\\\\
Now, since the red path is shorter than the blue path, the ants following the red path arrive at vertex $v_4$ before the ants following the blue path. Hence, on the way back to vertex $v_1$, the red path has more pheromone than the blue path because, the ants following the blue path have still not yet arrived at vertex $v_4$. Therefore, the ants that arrived at $v_4$ using the red path path would probabilistically choose to travel to $v_1$ using the red path again. As a result, the red path keeps on accumulating more pheromone than the blue path, and eventually this would lead to a situation were all ants choose the red path when traveling from $v_1$ to $v_4$ and vice-versa. Note that the red path is the shortest path of $G$.
\end{example}
After describing the ACO framework, it is now time to describe how the ACO framework can be applied to the TSP.  Let $G(V,E,f)$ be an instance of the TSP. As described in Example \ref{explain_ACO}, the ant's environment can be represented by $G$. In addition to this, since the goal of the TSP is to find a Hamiltonian cycle, it doesn't matter which is vertex representing the food source, and which is the vertex representing the nest, because, all vertices have to be visited. What is important is that when an ant starts walking in a graph it doesn't move to vertices that it has already visited, and after visiting all vertices it returns to the starting vertex. This can be done by giving each ant a working memory, to remember which cities it has visited. Note that although real ants do not posses this property, this property is a must in order for ants to compute Hamiltonian cycles. \cite{dorigo_gambardella_1997}\\\\
Therefore, at the start of the algorithm, each ant can be placed on a random vertex of $G$ (where $G$ is the TSP instance). At each time step, ants are moved around $G$ using a probabilistic function until they all compute a Hamiltonian cycle. For the TSP, the probabilistic function must be a function of both the pheromone accumulates on edges, and of the length of the edges connecting the current vertex the ant is on with a candidate vertex. Note that in this case a candidate vertex is a vertex which is not in the working memory of the ant (i.e. an unvisited vertex). Due to this probabilistic function, ants probabilistically prefer to move to vertices that are connected by edges with high pheromone and which are close-by. This will become clearer when presenting the probabilistic function itself.  Another important part of the ACO algorithm is local trail updating. Local trail updating is the reduction of pheromone on the edges chosen by an ant at each time step. As it will be seen from the formula, local trail updating is important because, it avoids having an ant choosing a vertex with a high probability of being chosen. As a result, this increases the number of Hamiltonian cycles that can be explored, i.e. increases the chance of finding a minimum weight Hamiltonian cycle. Note that as will be described later, local trail updating mimics the idea of trail evaporation in real ants. In the ACO framework it was discussed that pheromone would eventually accumulate on shorter paths. Therefore, for the ACO algorithm to posses this property, the ant that produces the best solution $S$ in a single iteration will be made to add more pheromone on the edges constituting $S$. This procedure is termed as global trail updating. Global trail updating will eventually help accumulate pheromone on shorter paths, thus, since ants probabilistically prefer shorter edges with high pheromone, they would eventually take the shorter paths. Note that, there are different ways how global trail updating and local trail updating can be performed, were the different ways represent a different variant of the ACO algorithm. In this write up the Ant Colony System (ACS) variant of the ACO algorithm will be considered due to the main theorem of this section. Later in this section, other variants of the ACO algorithm will be briefly described by discussing how they very from the ACS.\cite{dorigo_gambardella_1997}\\\\
From what have been discussed above, an ACO algorithm for the TSP can therefore be represented by the pseudocode in Algorithm \ref{ACO_ALG} below. Note that Algorithm \ref{ACO_ALG} was constructed using ideas from \cite{dorigo_gambardella_1997}. In the following algorithm, $G(V,E,f)$ is an instance of the TSP, and $g$ is a probabilistic function as described in the previous paragraph. Also, $a.current\_vertex$ is the variable containing the vertex the ant $a$ is currently on.
\begin{algorithm}[H]
\begin{algorithmic}[1]
\State best = $\{\}$
\State best.weight = $\infty$
\While{(!finished)}
          \State Place $m$ new ants on $m$ randomly selected vertices of $V$.
		...
		\EndFor
	\EndWhile
	\State $A$ = ant which produces the minimum weight Hamiltonian cycle of this iteration.
	\State Perform global trail updating on the Hamiltonian cycle computed by $A$.
	\If{best.weight $\geq$ A.Hamiltonian\_Cycle.weight}
		\State best = A.Hamiltonian\_Cycle
	\EndIf
\EndWhile
\State Return best
\caption{: ACO($G(V,E,f)$)}
...
\end{algorithmic}
\end{algorithm}
For Algorithm \ref{ACO_ALG} above to be implemented, there remains to define the probabilistic function $g$ which the ants use in order to move to another vertex, and the global/local trail updating formulas. As mentioned in this section, there are different ways how these three formulas can be defined for the TSP. However, as stated before, in this write up the ACS approach will be undertaken. Therefore, if $G(V,E,f)$ is a TSP instance, an ant $k$ in vertex $r$ chooses the city $s$ to move to among those vertices which do not belong to its working memory $M_k$, by applying the probabilistic formula below. Note that this formula was suggested by  Dorigo and Gambardella in \cite{dorigo_gambardella_1997}.
\begin{equation}
  \label{eq:aco1}
  s=\begin{cases}
...
% say different ACO algorithms
%check all presented before going to theoratic.
%go to theoratic, why no time complexity and no proofs,
%move to proof .. also say why th eproblem in previous algorithms graph will not be done
%must give good check, use book to give more detail, either update global best tour so far aor not.. give paramters best results.
\newpage
