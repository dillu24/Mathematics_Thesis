...
Similarly to the NNA, the Twice Around the Minimum Spanning Tree Algorithm (TAMSA) is an approximation algorithm that can find near-optimal solutions for the TSP. Before describing the internal workings of the TAMSA, two algorithms that are used by the TAMSA need to be briefly described. Note that, the internal workings of these algorithms will not be described because, it is out of the scope of this section.\\\\
The first algorithm used by the TAMSA is Prim's algorithm. Prim's algorithm takes a weighted graph $G(V,E,f)$ and an arbitrary starting vertex $r$ $\in$ $V$ as inputs, and returns a minimum weight spanning tree $T(V^\prime,E^\prime,f)$ of G. T is computed by starting with an empty set of edges and an empty set of vertices. Then, starting by adding $r$ to $V^\prime$, vertices and edges are added to $V^\prime$ and $E^\prime$ respectively such that, the edges added are always the least possible connecting two distinct vertices were one vertex is already in the $V^\prime$, and the other is not in $V^\prime$. This procedure continues until all vertices are added to $V^\prime$. Note that, choosing a different starting vertex from $G$ does not change the minimum weight spanning tree computed by Prim's algorithm.  \cite{cormen_leiserson_rivest_stein}\\\\
What follows is an example that demonstrates how Prim's algorithm constructs a minimum weight spanning tree for an arbitrary graph.
\begin{example}
\label{example_prim}
\normalfont{Consider} the graph $G$ in Example \ref{example4}. Let $T(V^\prime,E^\prime)$ be an empty graph. Assuming the starting vertex is $v_1$, Prim's algorithm starts by adding $v_1$ to $V^\prime$ which results in the following graph.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
    ...
    \node[state] (v1) {$v_1$};
\end{tikzpicture}\\\\
The algorithm now proceeds by adding the edge of least weight from $E$ to $E^\prime$ connecting a vertex $v$ $\in$ $V$ to a vertex $v^\prime$ $\in$ $V^\prime$. Also, $v$ is added to $V^\prime$. In this example, these properties are satisfied by the edge $\{v_1, v_2\}$, hence, resulting in the following graph.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
    ...
    \path[->] (v1) edge[]  node[]{4}(v2);
\end{tikzpicture}\\\\
Since not all vertices in $V$ have been added to $V^\prime$, the same procedure is repeated. Hence, the edge $\{v_2, v_3\}$ is chosen because, it is the least weight edge connecting a vertex in $V$ to a vertex in $V^\prime$. This results in the following graph\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
    ...
    \path[->] (v2) edge[]  node[pos=0.2,below right]{2} (v3);
\end{tikzpicture}\\\\
Again, since not all vertices in $V$ have been added to $V^\prime$, the same procedure is repeated, were the edge $\{v_2, v_4\}$ is the edge of least weight connecting a vertex in $V$ to a vertex in $V^\prime$.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
    ...
    \path[->] (v2) edge  node[]{4}(v4);
\end{tikzpicture}\\\\
Since all vertices in $V$ have been added to $V^\prime$, the algorithm now terminates by returning the spanning tree above.
\end{example}
