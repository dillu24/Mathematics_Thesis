...
Let G(V,E,f) be a complete weighted graph on $n$ vertices, and $C^\star$ be the optimal value of the Metric-TSP defined on G. Suppose that there exists a mapping f : V $\mapsto$ $\real$, f(v) = $l_v$, such that the following  two conditions hold:
\begin{itemize}
...
\end{claim}
Let $H(V^\prime,E^\prime)$ be the complete subgraph of $G$ such that, $V^\prime$ = $\{i | 1 \leq i \leq min(2k, n)\}$.
\end{proof}
\begin{theorem}
