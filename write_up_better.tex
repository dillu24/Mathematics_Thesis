...
\end{example}
Reasoning about computational problems is usally done by looking at the properties that are possed by the algorithms that solve these problems. One property that is sometimes looked at is called the running time of an algorithm. This is defined in definition \ref{running_time} below.
\begin{definition}
\label{running_time}
The running time of an algorithm is the number of steps taken by the algorithm to execute given a particular input. The running time is represented by the function T $:$ $\mathbb{N}$ $\mapsto$ $\mathbb{N}$ were the domain represents the size of the input, and the co domain reresents the amount of steps taken by the algorithm to execute given a particular input size. {\normalfont{\cite{cormen_leiserson_rivest_stein}, \cite{adamchik_2009}}}
\end{definition}
The following is an example on how the running time of an algorithm can be constructed.
\begin{example}
\label{example_running}
{\normalfont{Consider the addition of 2 bits. Then if the addition of 2 bits takes}} a {\normalfont{computational steps, the addition of 2}} $\mathit{n-}${\normalfont{bit integers will take}} a $\times$ n {\normalfont{steps. Therefore the running time of this algorithm for an input of size n is}} T(n) = a $\times$ n. {\normalfont{\cite{cormen_leiserson_rivest_stein}, \cite{adamchik_2009}}}
\end{example}
Expressing the running time in this way seems pretty easy to understand, however it has problems. Typically, the running time of an algorithm is fixed for any given input, however, the running time function is not defined on the actual inputs but on the input sizes. This means that since the same input size might represent different inputs, the running time function may be different for the same input size $\mathit{n}$, as this depends on which input is given. Therefore to solve this problem, the worst-case running time is usually used when reasoning about the running time of an algorithm. The worst-case running time is the longest running time for any input of size $\mathit{n}$. This would solve the problem because, the worst-case running time of an algorithm is an upper bound to all other running times for any input of size $\mathit{n}$, therefore, this assures that the algorithm will never get a longer running time for any input of size $\mathit{n}$. This avoids making guesses about the running times and hoping that it would never get worse.\cite{cormen_leiserson_rivest_stein}, \cite{adamchik_2009}\\\\
Another problem is that, the running time can be difficult to compute precisely because it may differ on different computers. This informally means that the running time of a particular algorithm would need a different amount of computational steps on different computers. For example the addition of 2 bits as seen in example \ref{example_running}, might take a different number of computational steps on different computers resulting into different running times. To get around this problem, the algorithms are analyzed asymptotically. This means making the input size increase without bound so that in the running time function, only the order of growth of the function is relevant. This can only be done because as the input size increases, the lower-order terms and multiplicative constants of the running time get effected by the size of the input itself. This means that given any running time function, the lower order terms and multiplicative constants of the function can be ignored. As a result in a running time function, the highest order term is the most relevant because it gives the order of growth of the function, i.e. how will the function behave as the input size grows without bound. For example, given the running time function $\mathit{T(n) = an^2+bn+c}$, what remains in the asymptotic analysis of this running time function is $\mathit{n^2}$. It is important to note that the order of growth of different running time functions on different machines will not differ on different machines as this is directly dependent on the size of the input. For example, the $\mathit{n}$-bit addition algorithm in example \ref{example_running} may have a running time function $\mathit{T(n) = a \times n}$ on one computer and a running time function $\mathit{T(n) = b \times n}$ on another computer depending on the number of computational steps . However, the order of growth is $\mathit{n^2}$ for both.  \cite{cormen_leiserson_rivest_stein}, \cite{adamchik_2009} \\\\
To express the asymptotic behaviour of functions, asymptotic notations are normally used. One of these notations is called the Big O Notation, whose exact definition is given below.
\begin{definition}
\label{bigonotation}
Give big o definition , example of it , what it means , give some running time functions and go to np ..check what is time complexity.. structure the above better and dowhat remains exactly
\end{definition}
\newpage
