...
\end{proof}
The proof for Theorem \ref{no_approx_tsp} was constructed using ideas in \cite{cormen_leiserson_rivest_stein}.\\\\
Since we do not know whether P=NP, Theorem \ref{no_approx_tsp} implies that no approximation algorithm with a constant approximation ratio for the TSP is known. In addition to that, we are not able to show that such an algorithm does not exist. In the following subsection, two approximation algorithms suitable for the TSP will be presented along with theoratic results about them. These are the Nearest Neighbour Approximation Algorithm and the Twice Around The Minimum Spanning Tree Approximation Algorithm.
...
\end{itemize}
The idea behind the NNA is to try and find a minimum weight Hamiltonian cycle by always moving from one vertex to another using the edges of least weight. This means that the NNA tries to choose an edge which looks the best at that point in time, hoping that it would lead to a minimum weight Hamiltonian cycle. However, this does not mean that the optimal solution for the TSP will be found. It is important to note that by Definition \ref{hamiltonian cycle}, the sequence of vertices outputted by the NNA are always a Hamiltonian cycle. This is true because, all the vertices in the graph must be visited before the algorithm terminates. Also, excluding the starting vertex, each vertex cannot be visited more than once because, by bullet point number two in the above procedure, the algorithm only chooses edges that lead to unvisited vertices. The following example is constructed to show how the NNA constructs an approximate solution to the TSP.
\begin{example}
\label{example_nna_explanation}
\normalfont{Consider the graph} $G$ in \ref{example4}. By Example \ref{example_2.1}, the solution to the TSP given $G$ is the Hamiltonian cycle $C$ = $v_1$, $v_3$, $v_2$, $v_4$, $v_1$ with weight 18. Suppose that we try to find $C$ using the NNA. Suppose that randomly the NNA has chosen to start from vertex $v_1$. The following pictorial graphs represent how the NNA will progress step by step using the NNA procedure described above. In these graphs, the blue vertices are the visited vertices, and black vertices are the unvisited vertices. Also, the chosen edges will be coloured red whilst the unchosen edges will be coloured black.\\\\
\underline{Step 1:}\\\\
...
\end{tikzpicture}\\\\
Finally, the NNA returns the Hamiltonian cycle $v_1$, $v_2$, $v_3$, $v_4$, $v_1$ of weight 23. Clearly this cycle is not the minimum weight Hamiltonian cycle. The minimum weight Hamiltonian cycle would have been found if the NNA was made to start from vertex $v_2$
\end{example}
Example \ref{example_nna_explanation} seems to indicate that the approximate solution returned by the NNA is good enough. In fact, the difference in weight between the approximate solution returned by the NNA and the optimal solution is just five. However, there could be cases where the NNA returns very bad approximations. The following example shows when the NNA can give bad approximate results
\begin{example}
\label{nna_fail}
\normalfont{Let} $M \in \mathbb{Z}^+$ such that $M$ is a very large number. Consider the graph $G$ below:
\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    ...
    \path[->] (v4) edge[]  node[pos=0.2,below left]{7}(v1);
\end{tikzpicture}\\\\
Clearly the minimum weight Hamiltonian cycle of $G$ is $v_2, v_3, v_1, v_4, v_2$ of weight 18. \\It is important to note that, if the edges differ a lot in the weight assigned, the NNA might give a bad approximation of the minimum weight Hamiltonian cycle. In fact, if the NNA starts from vertex $v_2$ the minimum weight Hamiltonian cycle is found. On the other hand, if the algorithm starts from vertex $v_1$, the NNA returns the Hamiltonian cycle $v_1$, $v_2$, $v_3$, $v_4$, $v_1$ of weight $M + 13$. This means that, since $M$ is a very large number, the NNA would return an approximate result that differs a lot in weight compared to the optimal solution.
\end{example}
Therefore, due to Example \ref{nna_fail}, it can be concluded that the result returned by the NNA depends heavily on which vertex the algorithm starts with. As discussed in Example \ref{nna_fail}, this dependency is mostly seen when the weights differ a lot in size.

An easy fix to this problem would be to restart the algorithm from different vertices and choosing the vertex (algorithm good with small number of vertices, easy to program, time complexity before Is better, then define metric tsp and bounds on it.
