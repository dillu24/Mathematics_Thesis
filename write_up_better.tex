...
The first algorithm used by the TAMSA is Prim's algorithm. Prim's algorithm takes a weighted graph $G(V,E,f)$ and an arbitrary starting vertex $r$ $\in$ $V$ as inputs, and returns a minimum weight spanning tree $T(V^\prime,E^\prime,f)$ of $G$. Prim's algorithm computes $T$ in the following way. The algorithm initially starts with empty $V^\prime$ and empty $E^\prime$. Then, the algorithm proceeds by adding $r$ to $V^\prime$. Afterwards, the algorithm adds to $E^\prime$, the edge of least-weight connecting a vertex $v$ in $V$ to a vertex $v^\prime$ in $V^\prime$. Following this, $v$ is added to $V^\prime$. The described procedure is then repeated until all vertices in $V$ are added to $V^\prime$. \cite{cormen_leiserson_rivest_stein}\\\\
An important question that must be answered is whether the procedure described in the previous paragraph does return a minimum weight Hamiltonian cycle. Typically, one needs to show that the mathematical structure returned by Prim's algorithm is a spanning tree whose weight is the least possible among all possible spanning trees of an input graph. Note that this consideration is important because, if Prim's algorithm does not return a minimum weight spanning tree, the TAMSA cannot use it in it's implementation. The next two theorems confirm that for an arbitrary input graph, Prim's algorithm returns a minimum weight spanning tree for that input graph.
\begin{theorem}
\end{theorem}
\begin{proof}
\end{proof}
