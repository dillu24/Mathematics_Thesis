...
The first algorithm used by the TAMSA is Prim's algorithm. Prim's algorithm takes a weighted graph $G(V,E,f)$ and an arbitrary starting vertex $r$ $\in$ $V$ as inputs, and returns a minimum weight spanning tree $T(V^\prime,E^\prime,f)$ of $G$. Prim's algorithm computes $T$ in the following way. The algorithm initially starts with empty $V^\prime$ and empty $E^\prime$. Then, the algorithm proceeds by adding $r$ to $V^\prime$. Afterwards, the algorithm adds to $E^\prime$, the edge of least-weight connecting a vertex $v^\prime$ in $V^\prime$ to a vertex $v$ in $V$ such that, $v$ is not in $V^\prime$ . Following this, $v$ is added to $V^\prime$. The described procedure is then repeated until all vertices in $V$ are added to $V^\prime$. \cite{cormen_leiserson_rivest_stein}\\\\
An important question that must be answered is whether the procedure described in the previous paragraph does return a minimum weight spanning tree. Typically, one needs to show that the mathematical structure returned by Prim's algorithm is a spanning tree whose weight is the least possible among all spanning trees of the input graph. Note that this consideration is important because, if Prim's algorithm does not return a minimum weight spanning tree, the TAMSA cannot use it in it's implementation. This will become clearer later on in this section when describing how the TAMSA uses Prim's algorithm. The next two theorems confirm that for an arbitrary complete weighted graph $G$, Prim's algorithm always returns a minimum weight spanning tree of $G$. Theorem \ref{correctness1} was constructed using ideas from \cite{prim's_algorithm}.
\begin{theorem}
\label{correctness1}
Prim's algorithm always returns a spanning tree. \cite{prim's_algorithm}
\end{theorem}
\begin{proof}
Let $G(V,E,f)$ be a complete weighted graph, and $T(V^\prime,E^\prime,f)$ be the graph returned by Prim's algorithm when given $G$ as input together with an arbitrary $s$ $\in$ $V$ as the starting vertex. To show that $T$ is a spanning tree, it must be shown that $T$ is a connected spanning sub graph of $G$ with no cycles. Note that $T$ is automatically a sub graph of $G$ because, Prim's algorithm adds only vertices and edges to $T$ belonging to $G$\\\\
Suppose that $T$ does not span $G$. Then, there exists a non-empty set of vertices $X$ $\subseteq$ $V$ such that, $X$ $\nsubseteq$ $V^\prime$. Let $x$ $\in$ $X$. It can be deduced that there are no edges in $E$ of the form $\{v,x\}$ where $v$ $\in$ $V^\prime$, otherwise, the Prim's algorithm would have chosen the edge and added $x$ to $V^\prime$. Now, since $G$ is connected, there exists a path $P$ joining $s$ to $x$. In addition to this, $s$ $\in$ $V^\prime$ and $x$ $\in$ $X$. Hence, $P$ must contain an edge of the form $\{v, x\}$ where $v$ $in$ $V^\prime$. Contradiction. Therefore, $T$ must be a spanning sub graph of $G$.\\\\
To show that $T$ is connected, it must be shown that any two distinct vertices in $V^\prime$ are connected by a path. Let $u$, $v$ $in$ $V^\prime$ such that, $u$ $\neq$ $v$. Now, before Prim's algorithm adds $u$ to $V^\prime$, it connects it with a vertex $u_k$ which is already in $V^\prime$. But before $u_k$ was added to $V^\prime$, Prim's algorithm must have connected $u_k$ to a vertex $u_{k=1}$ that is already in $V^\prime$. This process must terminate because, there are a finite number of vertices in $V$, hence, there are a finite number of vertices that are chosen by Prim's algorithm before $u$. Clearly, this process terminates when $u_0$ = $s$, because, $s$ is the first vertex chosen by Prim's algorithm. Now by this repeating process, $s= u_0, u_1, u_2, ..., u_k, u_{k+1} = u$ must be a path in $T$ joining $s$ to $u$ because, $\{u_i, u_{i+1}\}$ $\in$ $E^\prime$ for all $i$ $\in$ [$k$]. Now, the same argument can be repeated for $v$ and hence, $s$ and $v$ must also be connected by a path. Therefore, since both $u$ and $v$ connect to the same vertex $s$ via a Path, $u$ and $v$ must be connected by a path. Hence, $T$ must be a connected spanning sub graph of $G$.\\\\
Suppose that $T$ contains a cycle $C$. Let $\{c_1, c_2\}$ be the last edge of $C$ that was added by Prim's algorithm to $E^\prime$ (hence the edge that caused the cycle). WLOG, suppose that $c_1$ was chosen before $c_2$ by Prim's algorithm. Then, since Prim's algorithm adds an edge between a vertex in $V^\prime$ and a vertex not in $V^\prime$, it must be that $c_2$ is not in $V^\prime$ before $\{c_1, c_2\}$ is added to $E^\prime$. Hence, before $\{c_1, c_2\}$ is added to $E^\prime$, there are no paths joining $c_1$ to $c_2$ because $c_2$ is not in $V^\prime$. This means that, no cycle is created when adding $\{c_1, c_2\}$ to $E^\prime$ because, $c_1$ is connected to $c_2$ by exactly one path (the path $c_1, c_2$). Contradiction, since $\{c_1, c_2\}$ is the edge that caused the cycle to be created. Therefore, $T$ must be a connected spanning sub graph of $G$, hence, by Definition \ref{spanning tree}, $T$ is a spanning tree of $G$.
\end{proof}
Theorem \ref{correctness1} confirms that Prim's algorithm returns a spanning tree. Now it must be shown that the spanning tree returned by Prim's algorithm has the least possible weight among all spanning trees of $G$. Theorem \ref{correctness2} was adapted using ideas from %put reference here.
\begin{theorem}
\label{correctness2}
\end{theorem}
\begin{proof}
\end{proof}
What follows is an example that demonstrates how Prim's algorithm constructs a minimum spanning tree of a graph.
\begin{example}
\label{example_prim}
...
\begin{tikzpicture}[
    ...
    \node[state] (v1) {$v_1$};
\end{tikzpicture}\\\\
The algorithm now proceeds by adding the edge of least weight from $E$ to $E^\prime$ which connects a vertex $v$ $\in$ $V$ to a vertex $v^\prime$ $\in$ $V^\prime$. Also, $v$ is added to $V^\prime$. In this example, these properties are satisfied by the edge $\{v_1, v_2\}$, hence, $T$ is the following graph.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
    ...
    \path[->] (v1) edge[]  node[]{4}(v2);
\end{tikzpicture}\\\\
Since not all vertices in $V$ have been added to $V^\prime$, the same procedure is repeated. Hence, the edge $\{v_2, v_3\}$ is chosen because, it is the edge of least weight connecting a vertex in $V$ to a vertex in $V^\prime$. Therefore, $T$ is the following graph\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
    ...
    \path[->] (v2) edge[]  node[pos=0.2,below right]{2} (v3);
\end{tikzpicture}\\\\
Again, since not all vertices in $V$ have been added to $V^\prime$, the same procedure is repeated. In this case, $\{v_2, v_4\}$ is the edge of least weight connecting a vertex in $V$ to a vertex in $V^\prime$. Therefore, $T$ is the following graph.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
...
\end{example}
The second algorithm used by the TAMSA is the preorder tree walk algorithm. Given a tree $T(V,E,f)$ and an arbitrary starting vertex $r$ as inputs, the preorder tree walk algorithm returns a walk $S$ of $T$ containing all vertices in $V$, ordered according to how the vertices were visited during the walk. The preorder tree walk algorithm computes a walk $S$ in the following way. At the start of the algorithm, $S$ is empty. Then, starting from $r$, at any vertex $u$ the algorithm adds $u$ to $S$, and recursively calls itself on all neighbours of $u$. The entire procedure is repeated until all vertices in $V$ have been added to $S$. Note that in this section, the execution of the preorder tree walk algorithm on an arbitrary tree will be termed as a preorder walk. For example, a preorder walk of the tree in Example \ref{tree_example} below, results in the vertex sequence $v_1, v_2, v_3, v_4, v_5, v_9, v_6, v_7, v_8$. \cite{cormen_leiserson_rivest_stein}
\begin{example}
\label{tree_example}
\begin{tikzpicture}[
    > = , % arrow head style
...
\end{tikzpicture}\\\\
\end{example}
Having briefly described important algorithms that will be used by the TAMSA algorithm, now it is time to describe the actual TAMSA. The TAMSA takes as input an instance of the TSP, and returns a sequence of vertices representing a Hamiltonian cycle \cite{cormen_leiserson_rivest_stein}. The TAMSA algorithm can be described using the pseudocode adapted from \cite{cormen_leiserson_rivest_stein} below.\\\\
\textbf{TAMSA($G(V,E,f)$):}
\begin{itemize}
\itemsep0em
...
\item Perform a preorder tree walk of T starting from $r$, and let $S$ be the walk returned by the preorder tree walk algorithm.
\item Add $r$ to $S$ as a final vertex and return S.
\end{itemize}
For the TAMSA to be a useful approximation algorithm it must be checked that it does compute a Hamiltonian cycle, and that the Hamiltonian cycle is computed in polynomial time. Let $G$ be an instance of the TSP, and let $T$ and $S$ be the minimum weight spanning tree of $G$, and the walk returned by the preorder tree walk algorithm respectively. Now, since $T$ has no cycles, a preorder tree walk of $T$ can never result into a walk with repetition of vertices. Hence, $S$ has no repetiton of vertices. In addition to that, the preorder tree walk algorithm terminates when all vertices of $G$ have been added to $S$, hence, since $T$ is connected, it must contain all vertices of $G$. Therefore, $S$ must be a path containing all vertices of $G$. Therefore, for $S$ to be a Hamiltonian cycle, the first vertex in $S$ must be repeated as a final vertex. This is done by bullet point number four in the TAMSA pseudocode. Hence $S$ must be a Hamiltonian cycle. The next step is to check that the TAMSA computes a Hamiltonian cycle in polynomial time.\\\\
Given a TSP instance $G(V,E,f)$, the time complexity of the TAMSA is $O(|V|^2)$ \cite{cormen_leiserson_rivest_stein}. This is true because of the following. Consider the TAMSA pseudocode above. Clearly, the largest amount of work to be done by the TAMSA algorithm is to compute a minimum weight spanning tree $T$ from $G$, and perform a preorder tree walk of $T$. The preorder tree walk of $T$ takes $|V|$ steps because, it requires to visit the entire set of vertices. Hence, bullet point number three takes $O(|V|)$ time. Now, according to Exercise 23.2-2 in \cite{cormen_leiserson_rivest_stein}, Prim's algorithm takes $O(|V^2|)$ time to compute a minimum spanning tree. Thus, the total time complexity of the TAMSA is $O(|V^2|)+ O(|V|) = O(|V^2|)$. However, in \cite{cormen_leiserson_rivest_stein} it is argued that the asymptotic time complexity of Prim's algorithm can be improved to $O(|E|+|V| log(|V|))$. Hence, this would mean that the time complexity of the TAMSA is no longer $O(|V^2|)$, but is $O(|E|+|V| log(|V|))$. This means that, the TAMSA has a log-linear time complexity. Therefore, using Table \ref{tab:table1} and and the fact that the NNA has a quadratic time complexity (discussed in Section 2.1.1), the TAMSA is more efficient asymptotically, because it has a smaller order of growth. Note that the analysis of the time complexity of Prim's algorithm is not given because, it requires deeper theory on data structures such as Fibonacci Heaps which is out of the scope of this project. What follows is an example demonstrating how the TAMSA computes a minimum weight Hamiltonian cycle.
\begin{example}
\label{tamsa_demonstration}
\normalfont{Consider} the graph $G$ in Example \ref{example4}, and the TAMSA pseudocode presented in this section. Assume that randomly, the TAMSA chooses vertex $v_1$ as starting vertex. From Example \ref{example_prim}, the minimum spanning tree $T$ returned by Prim's algorithm starting from vertex $v_1$ is the graph below.\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
    ...
    \path[->] (v2) edge  node[]{4}(v4);
\end{tikzpicture}\\\\
Now, according to bullet point number three in the TAMSA pseudocode, the TAMSA must perform a preorder tree walk on $T$. Refering to the brief description on the preorder tree walk algorithm, a preorder walk is computed in the following way. Note in the following graphs, the blue vertices are the visited vertices, whilst the black vertices are the unvisited vertices.\\
The algorithm first starts by visiting the starting vertex $v_1$ and adding it to $S$. Therefore, $S = v_1$.
\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    ...
    \path[->] (v2) edge  node[]{4}(v4);
\end{tikzpicture}\\\\
Since not all vertices have been visited, the algorithm recursively calls itself on the neighbours of $v_1$, which in this case is only $v_2$. Hence, $v_2$ is visited and added to $S$. Therefore $S= v_1, v_2$.
\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    ...
    \path[->] (v2) edge  node[]{4}(v4);
\end{tikzpicture}\\\\
Since not all vertices have been visited, the algorithm recursively calls itself on the neighbours of $v_2$, which in this case are $v_3$ and $v_4$. Assuming that $v_3$ is visited before $v_4$, $v_3$ is marked as visited and added to $S$. Therefore $S= v_1, v_2, v_3$.
\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    ...
    \path[->] (v2) edge  node[]{4}(v4);
\end{tikzpicture}\\\\
Since not all vertices have been visited, and $v_3$ has no neighbours, the algorithm now visits another neighbour of $v_2$ which in this case is $v_4$. Hence $v_4$ is marked as visited and added to $S$. Therefore $S= v_1, v_2, v_3, v_4$.
\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
...
\end{example}
From Example \ref{tamsa_demonstration} and Example \ref{example_nna_explanation}, one can observe that in this case both algorithms returned the same Hamiltonian cycle. In addition to this, similarly to the NNA, the TAMSA returns bad approximations for the graph presented in Example \ref{nna_fail}. This is true because of the following. Suppose that $G(V,E,f)$ is the TSP instance in Example \ref{nna_fail}, and $v_1$ is the vertex chosen randomly by the TAMSA as starting vertex. Then, since $M$ is a very large number, it will never be included in the minimum spanning tree produced by Prim's algorithm. In addition to this, since the weights on the other edges did not change, the minimum spanning tree produced by Prim's algorithm for $G$ is the same as that in Example \ref{tamsa_demonstration}. Hence the TAMSA, will return the Hamiltonian cycle $ H = v_1, v_2, v_3, v_4, v_1$. Now, since $f(\{v_3, v_4\})$ is very large, the weight of $H$ would be very large, hence differing a lot from the optimal solution. Note that this problem would not happen if the preorder traversal is started from vertex $v_4$. In fact, the Hamiltonian cycle returned by the TAMSA would be $H = v_4, v_2, v_3, v_1, v_4$ (assuming $v_3$ is visited before $v_1$ when considering $v_2$), where $H$ does not contain the edge $\{v_3, v_4\}$ of weight $M$. Therefore, again it can be concluded that the quality of the approximation returned by the TAMSA algorithm depends on the starting vertex.\\\\
Consider the TSP as defined in Definition \ref{TSP}. Similarly to the NNA, the approximation ratio of the TAMSA when applied to the TSP is not known. In addition to that, by Theorem \ref{no_approx_tsp}, it is known that the TAMSA is not a constant approximation algorithm for the TSP, unless P=NP. Therefore, since it is not known whether P=NP, it is not known whether the TAMSA is a constant approximation algorithm for the TSP. However, what is known is that, the TAMSA is a 2-approximation algorithm for the Metric-TSP \cite{cormen_leiserson_rivest_stein}. This fact is proved in Theorem \ref{2_approx} below. Before proving Theorem \ref{2_approx}, the concept of a full walk will be explained because, it is going to be used in the proof.\\\\
Consider the preorder tree walk description provided in this section. Let $S$ be the Hamiltonian cycle to be returned by the preorder tree walk algorithm. Suppose that the preorder tree walk algorithm is currently at a vertex $u$. Then, after adding $u$ to S, the preorder tree walk algorithm recursively calls itself on the neighbours $v_1, ..., v_k$ of $u$. Suppose that $v_1$ is the first neighbour of $u$ that is visited. After the entire subgraph connected to $u$ is visited, the algorithm must return to the vertex $u$ to recursively call itself on the next neighbouring vertex (in this case $v_2$). In a preorder tree walk, when returning to $u$, $u$ is not added again to $S$. However, in a full walk, $u$ is again added to $S$. Therefore, a full walk of a tree lists every vertex $u$ whenever $u$ is first visited, and whenever $u$ is returned to after visiting one of the subgraph connected to $u$ \cite{cormen_leiserson_rivest_stein}. In other words, a full walk is a preorder tree walk which lists a vertex every time it is visited in the order of how the vertices are visited. For example, a full walk of the tree in Example \ref{tree_example} would result into the walk, $v_1, v_2, v_3, v_2, v_4, v_2, v_1, v_5, v_1, v_9, v_6, v_9, v_7, v_9, v_8, v_9, v_1$. Note that in a full walk, each edge is traversed exactly twice. This is true because, at each vertex $u$, it uses an edge $e$ to traverse to a neighbour of $u$, and uses the same edge $e$ when returning back to vertex $u$. Having described what a full walk is, it is now time to prove that the TAMSA is a 2-approximation algorithm when applied to the Metric-TSP. Note that, the proof for Theorem \ref{2_approx} below was adapted from \cite{cormen_leiserson_rivest_stein}.
\begin{theorem}
\label{2_approx}
The TAMSA is a 2-approximation algorithm for the Metric-TSP {\normalfont{\cite{cormen_leiserson_rivest_stein}}}.
\end{theorem}
\begin{proof}
Let $G(V,E,f)$ be an instance of the Metric-TSP, and $H^\star$ be the minimum weight Hamiltonian cycle of $G$. In addition to this denote the weight of any subgraph $H^\prime$ as $w(H^\prime)$. A spanning tree $T^\star$ can be obtained from $H^\star$ by deleting an edge from $H^\star$. This is true because, when deleting an edge from $H^\star$, no vertices of $G$ are deleted, hence, all vertices in $V$ would still be in $T^\star$. In addition to that, the cycle in $H^\star$ would be broken. Hence by Definition \ref{spanning tree}, $T^\star$ would be a spanning tree. Now, since every edge weight is positive, deleting an edge from a Hamiltonian cycle would result into a spanning tree of lesser weight. But, the weight of any spanning tree is greater than that of the minimum weight spanning tree. Hence, the minimum weight spanning tree gives a lower bound on the weight of the minimum weight Hamiltonian cycle. Therefore, letting $T$ be the minimum weight spanning tree of $G$, this inequaltiy holds :
\begin{equation}
    \label{eq:1}
    w(T) \leq w(T^\star) \leq w(H^\star)
  \end{equation}
Now, let $W$ be a full walk of $T$. It was argued in this section that, a full walk uses an edge of $T$ exactly twice. Hence,
\begin{equation}
    \label{eq:2}
    w(W) = 2w(T) \leq 2w(H^\star) ({\normalfont{By Equation \ref{eq:1}}})\\
    \implies w(W) \leq 2w(H^\star)
  \end{equation}
However, $W$ is not a Hamiltonian cycle because it contains repetition of vertices. But, it can be noted that since $G$ satisfies the triangle inequality, travelling from vertex $u$ directly to a vertex $v$ has less weight then travelling from a vertex $u$ to a vertex $c$ and then travelling again to vertex $v$. Hence, the repetitions of vertices can be removed to obtain a Hamiltonian cycle $H$. Note that, $H$ uses a subset of the edges from $W$, hence,
\begin{equation}
    \label{eq:3}
    w(H) \leq w(W)
  \end{equation}
In addition to this, H is the preorder tree walk of the minimum weight spanning tree $T$. Hence, $H$ is the Hamiltonian cycle outputted by the TAMSA. Therefore, from Equation \ref{eq:3} and Equation \ref{eq:2} it can be concluded that,
\begin{equation}
    \label{eq:3}
    w(H) \leq 2w(H^\star)
  \end{equation}
Hence, the value of the approximate solution returned by the TAMSA is within a constant factor of 2 of the optimal value. Hence, the TAMSA is a 2-approximation algorithm for the Metric-TSP.
\end{proof}
Therefore, it is guaranteed by Theorem \ref{2_approx} that, the value of the approximation returned by the TAMSA for the Metric-TSP is never more than twice the optimal value, even for very large instance sizes. Recall that, this was not the case for the NNA. In fact, in Section 2.1 it was argued that, for large instance sizes, since the approximation ratio increases logarithmically, the approximation ratio becomes large enough that it may allow approximate solutions with a value that is far off from the optimal value. That being said, it is stated in \cite{cormen_leiserson_rivest_stein} that although the TAMSA has a constant approximation ratio, it is not the best practical choice for the Metric-TSP. In fact, Christofides gave a 3/2-approximation algorithm for the Metric-TSP. However, this will not be discussed in this project because, it requires presenting mathematical concepts such as minimum weight perfect matchings which are out of the scope of this project.\\\\
In Section 2.1.1 and Section 2.1.2, the aim was to present algorithms that approximate the TSP, and presenting their approximation ratios for special cases of the TSP. Although knowing the approximation ratio of an algorithm is important, it is more convenient in practice to design an algorithm that is guaranteed to find an optimal solution if given enough time, even if it's approximation ratio is not known. An example of an algorithm that is guaranteed to find an optimal solution if given enough time is the Ant Colony Optimization algorithm. This algorithm will be presented in the next subsection, and the main result in this section guarantees that the Ant Colony Optimization algorithm will always find an optimal solution if given enough time. 
\subsubsection{The Ant Colony Algorithm}
\newpage
