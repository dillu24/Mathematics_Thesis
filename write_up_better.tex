...
\begin{table}[H]
    ...
    \end{tabular}
\end{table}
Another important factor in the ACS is the termination condition. The termination condition determines when the ACS stops execution. In this implementation, the ACS algorithm stops executing after executing for 1000000 iterations. This was done in this way to ensure that the ACS is executed long enough before returning an approximation value. Having briefly described the ACS implementation, it is now time to empirically analyze the ACS.
\subsubsection{Results and Analysis of Results}
\label{acs_analysis}
%check how much sharp the bound is, and how much approximation ratio increases
%approximate timing
%do timing for (r^2 d) only
%say about terminating condition
