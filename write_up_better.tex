...
\end{example}
The second algorithm used by the TAMSA is the preorder tree traversal algorithm. Given a tree $T(V,E,f)$ and an arbitrary starting vertex $r$ as inputs, the preorder traversal algorithm returns a sequence of vertices ordered in the order the vertices were visited. The algorithm then starts from $r$ and performs the following steps. At some vertex $u$, the algorithm first adds $u$ to the sequence of vertices. Then, it recursively adds all the neighbours of $u$ to the sequence of vertices in the order they are visited. This procedure is repeated until all vertices have been added to the sequence of vertices. For example, a preorder traversal of the tree below results in the vertex sequence $v_1, v_2, v_3, v_4, v_5, v_9, v_6, v_7, v_8$. \cite{cormen_leiserson_rivest_stein}\\\\
\begin{tikzpicture}[
    > = , % arrow head style
    shorten > = 1pt, % don't touch arrow head to node
    auto,
...
\end{tikzpicture}\\\\
Having briefly described important algorithms that will be used by the TAMSA algorithm, now it is time to present the pseudocode that describes the TAMSA algorithm. This pseudocode was adapted from \cite{cormen_leiserson_rivest_stein}.\\\\
\textbf{TAMSA($G(V,E,f)$):}
\begin{itemize}
\itemsep0em
\item
