...
The Ant Colony Optimization Algorithm (ACO) forms part of the metaheuristic class of algorithms \cite{dorigo_stutzle_thomas_2004}. A metaheuristic is a set of ideas that can be used to define algorithms that solve a wide set of problems \cite{dorigo_stutzle_thomas_2004}. In other words this means that, the ACO is a framework that can be applied to a number of problems. Note that, this is different than what have been presented in Section 2.2.2 and Section 2.2.1 because, the TAMSA and the NNA are algorithms that are specifically written for a particular problem (in this case the TSP). Each metaheuristic has it's own general idea which can then be implemented specifically as an algorithm for a particular problem.\\\\
...
\end{algorithmic}
\end{algorithm}
For Algorithm \ref{ACO_ALG} above to be implemented, there remains to define the probabilistic function $g$ which the ants use in order to move to another vertex, and the global/local trail updating formulas. As mentioned in this section, there are different ways how these three formulas can be defined. However, as stated before, in this write up the ACS approach will be undertaken. Therefore, if $G(V,E,f)$ is a TSP instance, an ant $k$ in vertex $r$ chooses the city $s$ to move to among those vertices which do not belong to its working memory $M_k$, by applying the probabilistic formula below. Note that this formula was suggested by  Dorigo and Gambardella in \cite{dorigo_gambardella_1997}.
\begin{equation}
  \label{eq:aco1}
  s=\begin{cases}
...
% say different ACO algorithms, the next formula discused, local trail,important to avoid strong tours of being chosen, reduces tours by alpha, t_0 is usually not a large value.. say why evaporation is important here.
