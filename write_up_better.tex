...
Having presented the theory behind the NNA, the TAMSA, and the ACS algorithm, it is now time to check whether the results obtained in practice by these algorithms satisfy the theory. Therefore, an entire section will be dedicated to the computational results of the NNA, the TAMSA, and the ACS algorithm, where, this section has the following layout. Firstly, a brief description on the implementation of the NNA, the TAMSA, and the ACS will be given. This will also include a description of the values of any parameters that are needed to implement the algorithms. Secondly, the approximation ratios of the TAMSA, and the NNA stated by Theorem \ref{2_approx} and by Theorem \ref{log_bound_thrm} respectively, will be tested on Metric-TSP instances of different sizes. Thirdly, a comparison in terms of quality of approximations will be given between the results obtained by the TAMSA, the NNA, and the ACS. Finally, an experimental test will also be carried out on the ACS applied to the TSP with different distance types (ex euclidean distance, geographic distance etc). This experimental test will check how long in seconds does the ACS take, in order to find a solution of weight within a specified factor from the optimal value.
%tan NNA amel how it varies tas time passes habba log
%Experimental test on tsp acs different metrics
%note computational ghat tamsa ma tistax tamel geog habba li metric.
...
%Check that every algorithm is correct
\newpage
