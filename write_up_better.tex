...
\end{example}
After describing the ACO framework, it is now time to describe how an algorithm obeying the ACO framework can be written for the TSP. An ACO algorithm for the TSP can be represented by the following pseudocode. Note that the pseudocode below was adapted using ideas from \cite{dorigo_gambardella_1997}.\\\\
\textbf{ACO algorithm for the TSP pseudocode}\\\\
Suppose that $G(V,E,f)$ is an instance of the TSP, and $g$ is a probabilistic function.\\
\textbf{ACO($G(V,E,f)$):}\\
Place $m$ ants on $m$ randomly selected vertices.\\
while(!finished)\{\\
	for every ant $a$:
		move $a$ to vertex $v$ using the probabilistic function $g$.\\
		update the pheromone level of the edge $\{a.current\_vertex, v\}$\\
	$A$ = ant which produces the minimum weight Hamiltonian cycle of this iteration.\\
	Add more pheromone on the edges of the Hamiltonian cycle computed by $A$.\\
\}\\\\
%use above to state paragraph below, and explain.. sayclearer when representing equations .. or give immediately
 Let $G(V,E,f)$ be an instance of the TSP. As described in Example \ref{explain_ACO}, the ant's environment can be represented by $G$. In addition to this, since in the TSP a Hamiltonian cycle is to be found, it doesn't matter which is the food source vertex and which is the nest vertex because, all vertices have to be visited. Therefore initially, each ant can be placed on a random vertex of $G$. Moreover, suppose that an ant $a$ is at some vertex $u$. Ant $a$ chooses an unvisited vertex $v$ using a probabilistic function of both the pheromones accumulated on the edge $\{u,v\}$, and $f(\{u,v\})$. Hence, ants probabilistically prefer to move to vertices which are connected by an edge of high pheromone and which are close-by. It is important to avoid having a strong vertex chosen by all ants, otherwise, a very small number of Hamiltonian cycles will be explored. This can be done by decreasing the pheromone on a chosen edge, so that an ant has a higher probability of choosing another vertex. This is termed as local trail updating. When 

%describe pseudocode, move to artificial describe, formulas etc, indent .. say different ACO algorithms
%say why it obeys the ACO framework
