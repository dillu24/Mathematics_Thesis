...
\end{example}
Example \ref{example_nna_explanation} seems to indicate that the approximate solution returned by the NNA is good enough. In fact, the difference in weight between the approximate solution returned by the NNA and the optimal solution is just five. However, there could be cases where the NNA could fail badly. The following example shows where the NNA can give bad approximate results
\begin{example}
\label{nna_fail}
\end{example}
Therefore, from the above example it can be concluded that, the quality of the result returned by the NNA depends heavily on which vertex the algorithm starts on, especially when the edges differ a lot in weight. 

An easy fix to this problem would be to restart the algorithm from different vertices and choosing the vertex, time complexity before Is better, then define metric tsp and bounds on it. Maybe mention there is no constant approx algorithm known
